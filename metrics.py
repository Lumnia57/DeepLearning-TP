...
available_losses = ["FocalLoss", "CrossEntropyLoss", "WeightedCrossEntropyLoss"]
...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        _, num_classes = predictions.shape[:2]
        
        nb_classes = np.zeros(num_classes, dtype=int)
        t = targets.cpu().numpy().flatten()

        for c in range(num_classes):
            nb_classes[c] = np.count_nonzero(t==c)

        weights = torch.from_numpy((1.0-self.beta)/(1.0-self.beta**nb_classes)).type(torch.FloatTensor)

        return F.cross_entropy(predictions.cpu(), targets.cpu(), weight=weights)
...
if __name__ == "__main__":
    ...
    losses = [WeightedCrossEntropyLoss()]
    for l in losses:
