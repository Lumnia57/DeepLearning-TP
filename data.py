...
def test_dataset(args):
    ...
    plt.tight_layout()
    plt.savefig('fig-test-dataset.png')
...
def test_augmented_dataset(args):
    ...
    def data_transforms(img, mask):
        tf = A.Compose(
            [
                A.Normalize(0, 1),
                A.CoarseDropout(always_apply=True, max_height=60, max_width=60, max_holes = 10, min_holes = 1),
                A.HorizontalFlip(),
                ToTensorV2(),
            ]
        )
    ...
    plt.tight_layout()
    plt.savefig('fig-test-augmented-dataset.png')
